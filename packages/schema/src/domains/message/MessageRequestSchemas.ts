...
function omitEmbedObjectWithoutRequiredField(value: unknown, requiredField: string): unknown {
	...
	if (record[requiredField] === undefined || record[requiredField] === null) return undefined;
	return value;
}
...
export const RichEmbedAuthorRequest = z.preprocess((value) => omitEmbedObjectWithoutRequiredField(value, 'name'), RichEmbedAuthorRequestShape.nullish());
...
export const RichEmbedMediaRequestShape = z.object({
	...
	description: createStringType(1, RICH_EMBED_MEDIA_DESCRIPTION_MAX_LENGTH).nullish().describe('Alt text description of the media'),
});
export const RichEmbedMediaRequest = z.preprocess((value) => omitEmbedObjectWithoutRequiredField(value, 'url'), RichEmbedMediaRequestShape.nullish());
export type RichEmbedMediaRequest = z.infer<typeof RichEmbedMediaRequest>;

const RichEmbedFooterRequestShape = z.object({
	text: createStringType(1, RICH_EMBED_FOOTER_TEXT_MAX_LENGTH).describe(`Footer text (1-${RICH_EMBED_FOOTER_TEXT_MAX_LENGTH} characters)`),
	icon_url: URLType.nullish().describe('URL of the footer icon'),
});
export const RichEmbedFooterRequest = z.preprocess((value) => omitEmbedObjectWithoutRequiredField(value, 'text'), RichEmbedFooterRequestShape.nullish());
...
const RichEmbedFieldRequest = z.object({
	...
	value: createStringType(0, RICH_EMBED_FIELD_VALUE_MAX_LENGTH).describe(`Value of the field (0-${RICH_EMBED_FIELD_VALUE_MAX_LENGTH} characters)`),
...
export const RichEmbedRequest = z.object({
	...
	title: createStringType(0, RICH_EMBED_TITLE_MAX_LENGTH).nullish().describe(`Title of the embed (0-${RICH_EMBED_TITLE_MAX_LENGTH} characters)`),
	...
	description: z.preprocess(omitEmptyString, createStringType(1, RICH_EMBED_DESCRIPTION_MAX_LENGTH).nullish()).optional().describe(`Description of the embed (1-${RICH_EMBED_DESCRIPTION_MAX_LENGTH} characters)`),
	...
	fields: z.array(RichEmbedFieldRequest).max(RICH_EMBED_FIELDS_MAX).nullish().describe(`Array of field objects (max ${RICH_EMBED_FIELDS_MAX})`),
});
export type RichEmbedRequest = z.infer<typeof RichEmbedRequest>;

const MessageAuthorType = withOpenApiType(createNamedStringLiteralUnion([
	['user', 'user', 'A regular user account'],
	['bot', 'bot', 'An automated bot account'],
	['webhook', 'webhook', 'A webhook-generated message'],
], 'The type of author who sent the message'), 'MessageAuthorType');

const MessageContentType = withOpenApiType(createNamedStringLiteralUnion([
...
], 'The type of content contained in a message. Upload filters inspect attachments; embed filters inspect generated or supplied message embeds.'), 'MessageContentType');

const MessageEmbedType = withOpenApiType(createNamedStringLiteralUnion([
...
], 'The type of generated or supplied embed content, not uploaded attachments'), 'MessageEmbedType');

const MessageSortField = withOpenApiType(createNamedStringLiteralUnion([
	['timestamp', 'timestamp', 'Sort results by message timestamp'],
	['relevance', 'relevance', 'Sort results by search relevance score'],
], 'The field to sort search results by'), 'MessageSortField');

const MessageSortOrder = withOpenApiType(createNamedStringLiteralUnion([
	['asc', 'asc', 'Sort in ascending order (oldest/lowest first)'],
	['desc', 'desc', 'Sort in descending order (newest/highest first)'],
], 'The order to sort search results'), 'MessageSortOrder');

const MessageSearchScope = withOpenApiType(createNamedStringLiteralUnion([
	...
	['open_dms_and_all_guilds', 'open_dms_and_all_guilds', "Search across all DMs you currently have open and all Communities you're currently in"],
], 'Search scope for message searches'), 'MessageSearchScope');
...
export const MessageSearchRequest = z.object({
	...
	exclude_has: z.array(MessageContentType).max(20).optional().describe('Content flags the message must not have.'),
	embed_type: z.array(MessageEmbedType).max(20).optional().describe('Generated or supplied embed types to filter by; does not match uploaded attachments'),
	exclude_embed_type: z.array(MessageEmbedType).max(20).optional().describe('Generated or supplied embed types to exclude; does not match uploaded attachments'),
	...
	attachment_filename: z.array(createStringType(1, 1024)).max(100).optional().describe('Attachment filenames to filter by'),
	exclude_attachment_filename: z.array(createStringType(1, 1024)).max(100).optional().describe('Attachment filenames to exclude'),
	attachment_extension: z.array(createStringType(1, 32)).max(50).optional().describe('File extensions to filter by'),
	exclude_attachment_extension: z.array(createStringType(1, 32)).max(50).optional().describe('File extensions to exclude'),
	sort_by: MessageSortField.default('timestamp').describe('Field to sort search results by'),
...
export const GlobalSearchMessagesRequest = MessageSearchRequest.extend({
	context_channel_id: SnowflakeType.optional().describe('Channel ID for context when searching across multiple channels'),
...
export const MessageNonceRequest = z.union([
	createStringType(1, 32),
	z.number().int().nonnegative().safe().transform((value) => value.toString()),
]).pipe(createStringType(1, 32)).describe('Client-generated identifier for the message');
...
export const MessageRequestSchema = z.object({
	...
	attachments: z.array(z.union([ClientUploadedAttachmentRequest, ClientAttachmentRequest])).describe('Array of attachment objects'),
	poll: PollRequest.nullish().describe('Poll to create with this message'),
	message_reference: MessageReferenceRequest.nullish().describe('Reference to another message (for replies or forwards)'),
	allowed_mentions: AllowedMentionsRequest.nullish().describe('Controls which mentions trigger notifications'),
	flags: createBitflagInt32Type(MessageFlags, MessageFlagsDescriptions, 'Message flags bitfield', 'MessageFlags').default(0),
...
export const MessageEditRequestSchema = z.object({
	content: MessageContentRequest.nullish(),
	embeds: z.array(RichEmbedRequest).nullish(),
	attachments: z.array(ClientAttachmentReferenceRequest).nullish(),
	allowed_mentions: AllowedMentionsRequest.nullish(),
	flags: createBitflagInt32Type(MessageFlags, MessageFlagsDescriptions, 'Message flags bitfield', 'MessageFlags').optional(),
}).partial();
export type MessageEditRequestSchemaType = z.infer<typeof MessageEditRequestSchema>;

export const MessageDeleteBulkRequest = z.object({messages: z.array(SnowflakeType).min(2).max(100)});
export type MessageDeleteBulkRequest = z.infer<typeof MessageDeleteBulkRequest>;

export const MessageHistoryRequest = z.object({
	limit: createQueryIntegerType({min: 1, max: 100, defaultValue: 50}),
	before: SnowflakeType.optional(),
	after: SnowflakeType.optional(),
	around: SnowflakeType.optional(),
});
export type MessageHistoryRequest = z.infer<typeof MessageHistoryRequest>;

export const MessageReactionUsersRequest = z.object({
	limit: createQueryIntegerType({min: 1, max: 100, defaultValue: 25}),
	after: SnowflakeType.optional(),
});
export type MessageReactionUsersRequest = z.infer<typeof MessageReactionUsersRequest>;
