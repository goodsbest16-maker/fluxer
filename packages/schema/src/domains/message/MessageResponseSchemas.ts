...
export const PollAnswerResponse = z.object({
	...
	text: z.string().describe('Text shown for this poll answer'),
	attachment_id: SnowflakeStringType.nullish().describe('Optional image attachment associated with this answer'),
	vote_count: Int32Type.describe('Number of votes for this answer'),
	custom: z.boolean().describe('Whether this answer was added by a voter'),
});

export const PollResponse = z.object({
	question: z.string().describe('Poll title or question'),
	answers: z.array(PollAnswerResponse).describe('Answers and current vote totals'),
	closes_at: z.iso.datetime().nullish().describe('ISO 8601 timestamp when voting closes'),
	closed: z.boolean().describe('Whether voting is closed'),
	ranked: z.boolean().describe('Whether votes are ranked by preference'),
	...
	total_votes: Int32Type.describe('Total number of submitted vote records'),
	my_answer_ids: z.array(Int32Type).describe('Current user answer IDs in preference order'),
});
...
export const MessageSnapshotResponse = z.object({
	...
	mention_channels: z.array(MessageChannelMentionResponse).nullish().describe('Channels mentioned in the snapshot that are visible to @everyone'),
	...
	flags: createBitflagInt32Type(MessageFlags, MessageFlagsDescriptions, 'The bitwise flags of the original message', 'MessageFlags'),
});
...
const MessageBaseResponseSchema = z.object({
	...
	type: MessageTypeSchema,
	flags: createBitflagInt32Type(MessageFlags, MessageFlagsDescriptions, 'The bitwise flags for this message', 'MessageFlags'),
	...
	mention_channels: z.array(MessageChannelMentionResponse).nullish().describe('Channels mentioned in the message that are visible to @everyone'),
	users: z.array(z.lazy(() => UserPartialResponse)).nullish().describe('Users referenced from non-notifying content, embed, and snapshot text, included for client-side resolution'),
	...
	nsfw_emojis: z.array(SnowflakeStringType).optional().describe('IDs of custom emojis in this message that are classified as NSFW'),
	...
	poll: PollResponse.nullish().describe('Poll state and results when this message contains a poll'),
});
...
export const MessageResponseSchema = MessageBaseResponseSchema.extend({
	referenced_message: MessageBaseResponseSchema.nullish().describe('The message that this message is replying to or forwarding'),
});
const ChannelPinMessageResponse = MessageResponseSchema.omit({referenced_message: true, reactions: true}).describe('The pinned message');

export const ChannelPinResponse = z.object({message: ChannelPinMessageResponse, pinned_at: z.iso.datetime().describe('The ISO 8601 timestamp of when the message was pinned')});
export type ChannelPinResponse = z.infer<typeof ChannelPinResponse>;
export const ChannelPinsResponse = z.object({items: z.array(ChannelPinResponse).describe('Pinned messages in this channel'), has_more: z.boolean().describe('Whether more pins can be fetched with pagination')});
...
export const ReactionUsersPageResponse = z.object({items: z.array(z.lazy(() => UserPartialResponse)).describe('Users who reacted with the requested emoji'), has_more: z.boolean().describe('Whether more reaction users can be fetched'), next_after: SnowflakeStringType.nullable().describe('Cursor for the next page, or null when there are no more users')});
export type ReactionUsersPageResponse = z.infer<typeof ReactionUsersPageResponse>;
export const MessageSearchResultsResponse = z.object({messages: z.array(MessageResponseSchema.omit({referenced_message: true})).describe('The messages matching the search query'), channels: z.array(z.lazy(() => ChannelResponse)).describe('Serialized channels referenced by the returned messages'), total: Int32Type.describe('The total number of messages matching the search'), hits_per_page: Int32Type.describe('The maximum number of messages returned per page'), page: Int32Type.describe('The current page number'), cursor: z.array(z.string()).optional().describe('Opaque cursor for fetching the next page of results')});
export type MessageSearchResultsResponse = z.infer<typeof MessageSearchResultsResponse>;
const MessageSearchIndexingResponse = z.object({indexing: z.literal(true).describe('Indicates that one or more channels are being indexed')});
...
export const BulkMessageFetchResponse = z.object({channels: z.array(z.object({channel_id: SnowflakeStringType.describe('The ID of the channel whose messages were fetched'), messages: MessageListResponse.describe('Messages fetched for this channel')}))});
export type BulkMessageFetchResponse = z.infer<typeof BulkMessageFetchResponse>;

export interface MessageReference { readonly message_id: string; readonly channel_id: string; readonly guild_id?: string; readonly type?: number; readonly attachment_ids?: ReadonlyArray<string>; readonly embed_indices?: ReadonlyArray<number>; }
export interface ReactionEmoji { readonly id?: string | null; readonly name: string; readonly animated?: boolean; readonly url?: string | null; }
export interface MessageReaction { readonly emoji: ReactionEmoji; readonly count: number; readonly me?: true; readonly me_burst?: boolean; readonly count_details?: {readonly burst: number; readonly normal: number}; }
export interface MessageAttachment { readonly id: string; readonly filename: string; readonly title?: string; readonly description?: string; readonly content_type?: string; readonly size: number; readonly url: string | null; readonly proxy_url: string | null; readonly width?: number; readonly height?: number; readonly placeholder?: string; readonly flags: number; readonly duration?: number; readonly waveform?: string; readonly content_hash?: string | null; readonly nsfw?: boolean; readonly expires_at?: string | null; readonly expired?: boolean; }
export interface MessageCall { readonly participants: ReadonlyArray<string>; readonly ended_timestamp?: string | null; }
export interface MessageSnapshot { readonly type: number; readonly content: string; readonly flags?: number; readonly edited_timestamp?: string; readonly mentions?: ReadonlyArray<string>; readonly mention_roles?: ReadonlyArray<string>; readonly mention_channels?: ReadonlyArray<ChannelMention>; readonly embeds?: ReadonlyArray<MessageEmbed>; readonly attachments?: ReadonlyArray<MessageAttachment>; readonly stickers?: ReadonlyArray<MessageStickerItem>; readonly timestamp: string; }
export interface MessageStickerItem { readonly id: string; readonly name: string; readonly animated: boolean; readonly nsfw?: boolean; }
export interface AllowedMentions { readonly parse?: ReadonlyArray<'roles' | 'users' | 'everyone'>; readonly roles?: ReadonlyArray<string>; readonly users?: ReadonlyArray<string>; readonly replied_user?: boolean; }
export interface ChannelMention { readonly id: string; readonly type: number; readonly name: string; }
export interface MessageMention extends UserPartial { readonly member?: Omit<GuildMemberData, 'user'>; }
export interface PollAnswer { readonly id: number; readonly text: string; readonly attachment_id?: string | null; readonly vote_count: number; readonly custom: boolean; }
export interface MessagePoll { readonly question: string; readonly answers: ReadonlyArray<PollAnswer>; readonly closes_at?: string | null; readonly closed: boolean; readonly ranked: boolean; readonly anonymous: boolean; readonly allow_custom_answers: boolean; readonly total_votes: number; readonly my_answer_ids: ReadonlyArray<number>; }
